...
with tab9:
    ...
    if projects:
        df_stats = pd.DataFrame(projects)
        col1, col2 = st.columns(2)
        with col1:
            country_counts = df_stats["country"].value_counts()
            st.bar_chart(country_counts)
        with col2:
            tag_list = [tag for p in projects for tag in st.session_state["tags"][p["id"]]]
            tag_df = pd.DataFrame(Counter(tag_list).items(), columns=["Tag", "Count"])
            if not tag_df.empty:
